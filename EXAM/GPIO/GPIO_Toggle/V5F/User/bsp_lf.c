...
/* Seqlock-protected RANGE metadata so FLOW/IMU frames cannot overwrite the
 * notification for a newly decoded 0x34 frame. */
static volatile uint32_t s_lf_range_seq = 0UL;
static uint32_t s_lf_range_timestamp_ms = 0UL;
static uint32_t s_lf_range_sample_count = 0UL;
static uint32_t s_lf_range_last_cycle = 0UL;
static uint32_t s_lf_range_cycle_remainder = 0UL;
...
static uint32_t LF_CaptureRangeTimestampMs(void);
...
uint8_t LF_GetRangeSample(LF_RangeSample_t *out)
{
    ...
    for(;;)
    {
        seq_begin = s_lf_range_seq;
        if((seq_begin & 1UL) != 0UL)
        {
            continue;
        }

        __asm__ volatile("" ::: "memory");
        out->direction    = s_lf_data.range_direction;
        out->angle_deg    = s_lf_data.range_angle;
        out->distance_cm  = s_lf_data.range_distance_cm;
        out->valid        = s_lf_data.range_valid;
        out->timestamp_ms = s_lf_range_timestamp_ms;
        out->sample_count = s_lf_range_sample_count;
        __asm__ volatile("" ::: "memory");
        seq_end = s_lf_range_seq;
        if((seq_begin == seq_end) && ((seq_end & 1UL) == 0UL))
        {
            break;
        }
    }

    return (out->sample_count != 0UL) ? 1U : 0U;
}
...
static void LF_ResetRuntime(void)
{
    ...
    s_lf_data.frame_updated = 0U;
    s_lf_range_seq = 0UL;
    s_lf_range_timestamp_ms = 0UL;
    s_lf_range_sample_count = 0UL;
    s_lf_range_last_cycle = 0UL;
    s_lf_range_cycle_remainder = 0UL;
...
static void LF_DecodeFrame(const uint8_t *frame)
{
    ...
    switch(frame_id)
    {
        ...
        case LF_FRAME_ID_RANGE:
            ...
            s_lf_range_timestamp_ms = LF_CaptureRangeTimestampMs();
            s_lf_range_sample_count++;
            if(range_valid == 0U)
            {
...
/* Build a monotonic millisecond timestamp from the V5F hardware cycle
 * counter.  Unsigned subtraction also handles one 32-bit MCYCLE wrap; a
 * longer outage is harmless because V3F times out and reinitializes first. */
static uint32_t LF_CaptureRangeTimestampMs(void)
{
    uint32_t now_cycle = __get_MCYCLE();
    /* MCYCLE counts V5F core clocks, not the peripheral HCLK domain. */
    uint32_t cycles_per_ms = SystemCoreClock / 1000UL;
    uint32_t delta_cycles;
    uint32_t delta_ms;
    uint32_t remainder;

    if(s_lf_range_sample_count == 0UL || cycles_per_ms == 0UL)
    {
        s_lf_range_last_cycle = now_cycle;
        s_lf_range_cycle_remainder = 0UL;
        return 1UL;
    }

    delta_cycles = now_cycle - s_lf_range_last_cycle;
    s_lf_range_last_cycle = now_cycle;
    delta_ms = delta_cycles / cycles_per_ms;
    remainder = delta_cycles % cycles_per_ms;
    remainder += s_lf_range_cycle_remainder;
    if(remainder >= cycles_per_ms)
    {
        remainder -= cycles_per_ms;
        delta_ms++;
    }
    s_lf_range_cycle_remainder = remainder;

    /* A repeated marker must never make a decoded RANGE frame look old. */
    if(delta_ms == 0UL)
    {
        delta_ms = 1UL;
    }
    return s_lf_range_timestamp_ms + delta_ms;
}
